...
struct BestResponseEvaluator {
        ...
        int num_samples = 100000)
    {
        ...
        for(int iter = 0; iter < max_iters; ++iter){
            cout << "Iter " << iter << endl;
            // Q-values per (infoset, move) stored densely using DCFRPolicy's state indexing.
            // This avoids allocating millions of tiny vectors (which is both slow and memory-hungry).
            vector<double> Q_state(opponent_policy.state_count, 0.0);
