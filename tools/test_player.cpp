...
struct CliOptions {
    string policy_path = "./checkpoints/player0_7729319.bin";
    string seat = "random";
...
vector<float> getPolicyActionProbabilities(ThreeCardPlayer& bot,
    ...
    int info_set = bot.tree.calcInfoSet(node_id, hand, bot.flop, board, bot.opp_discard, bot.my_discard);
...
bool playHand(ThreeCardPlayer& bot, DynamicThreeCardBucket& bucket, std::mt19937& rng, int human_id, int hand_number){
    Deck deck(rng);
    uint64_t sb_hand = 0;
    uint64_t bb_hand = 0;
    uint64_t board = 0;
