...
struct ThreeCardInferenceTree {
    ...
    void prepare(int seed){
        ...
        for(int t = 0; t < sb_discard.size(); t++){
            ...
            int bb_bucket_10 = bucket->getFlopBucket(board[node_id], bb_hand, sb_discard_card[node_id], bb_discard_card[node_id], 10, 1);
            int sb_bucket_10 = bucket->getFlopBucket(board[node_id], sb_hand, bb_discard_card[node_id], sb_discard_card[node_id], 10, 0);
            if(nodes[node_id].getTurn() == 0){
                int sb_bucket = (buckets[node_id] == 1 ? 0 : sb_bucket_10);
                assert(sb_bucket < buckets[node_id]);
                nodes[node_id].setInfoSet(info_set_map[nodes[node_id].getInfoSetIndex()] + sb_bucket);
            } else {
                int bb_bucket = (buckets[node_id] == 1 ? 0 : bb_bucket_10);
                assert(bb_bucket < buckets[node_id]);
                nodes[node_id].setInfoSet(info_set_map[nodes[node_id].getInfoSetIndex()] + bb_bucket);
            }
            int l = node_id, r = node_id + nodes[node_id].getSize();
        ...
        for(int t = 0; t < deal_turn.size(); t++){
            ...
            int sb_bucket_10 = bucket->getTurnBucket(flop[node_id], board[node_id] ^ flop[node_id], sb_hand, bb_discard_card[node_id], sb_discard_card[node_id], 10, 0);
            int bb_bucket_10 = bucket->getTurnBucket(flop[node_id], board[node_id] ^ flop[node_id], bb_hand, sb_discard_card[node_id], bb_discard_card[node_id], 10, 1);
		...
		int bb_bucket = (buckets[node_id] == 1 ? 0 : bb_bucket_10);
            ...
            for(int i = l + 1; i <= r; i++){
                ...
                if(child_turn == 0){
		    int sb_bucket = (buckets[i] == 1 ? 0 : sb_bucket_10);
                    assert(sb_bucket < buckets[i]);
                    nodes[i].setInfoSet(info_set_map[nodes[i].getInfoSetIndex()] + sb_bucket);
                } else {
		    int bb_bucket = (buckets[i] == 1 ? 0 : bb_bucket_10);
                    assert(bb_bucket < buckets[i]);
                    nodes[i].setInfoSet(info_set_map[nodes[i].getInfoSetIndex()] + bb_bucket);
                }
            }
        ...
        for(int t = 0; t < deal_river.size(); t++){
            ...
            int sb_bucket_10 = bucket->getRiverBucket(flop[node_id], board[node_id] ^ flop[node_id], sb_hand, bb_discard_card[node_id], sb_discard_card[node_id], 10, 0);
            int bb_bucket_10 = bucket->getRiverBucket(flop[node_id], board[node_id] ^ flop[node_id], bb_hand, sb_discard_card[node_id], bb_discard_card[node_id], 10, 1);
